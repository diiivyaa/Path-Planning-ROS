...
std::vector<std::array<int, 2>> findPath(int idx_x, int idx_y, int goal_x, int goal_y)
{
  ...
  while (!queue.empty())
  {
    ...
    Node *cur_node = queue.front(); //  declare a structure pointer (cur_node) and assign the first element of the queue to the pointer
    queue.pop_front();              //  remove the first element from queue
    int cur_x = cur_node->x;        //  Decleare a new variable (cur_x, cur_y and cur_cost) and assign x, y and cost from cur_node pointer
    int cur_y = cur_node->y;
    int cur_cost = cur_node->cost;

    // ---------------------- (2) Find the path if goal is found, and break loop -------------------------
    if (cur_x == goal_x && cur_y == goal_y)   // if the current cell is the destination cell then the following code will be exeuted
    {
      Node *node = cur_node;                 // Declare a new pointer and point to same location as cur_node
      do
      {
        path.push_back({node->x, node->y}); // Push x and y values into path 
        node = node->parent;                // Retrieve parent cell and assign it node pointer
      } while (node != nullptr);            // Loop is executed til the deafult parent nullptr is retrieved
      break;
    }
    ...
    {                                              // north wall does not exist AND north cell exists ==> north cell is accessible
      ...
      int new_north_cell_cost = cur_cost + sqrt(pow(goal_x - cur_x + 1, 2) + pow(goal_y - cur_y, 2));
    ...
    {                                             // east wall does not exist AND east cell exists ==> east cell is accessible
      ...
      int new_east_cell_cost = cur_cost + 1;//sqrt(pow(goal_x - cur_x, 2) + pow(goal_y - cur_y - 1, 2));
    ...
    {                                             // west wall does not exist AND west cell exists ==> west cell is accessible
      ...
      int new_west_cell_cost = cur_cost + 1;//sqrt(pow(goal_x - cur_x, 2) + pow(goal_y - cur_y + 1, 2));
    ...
    {                                              // south wall does not exist AND south cell exists ==> south cell is accessible
      ...
      int new_south_cell_cost = cur_cost + 1;//sqrt(pow(goal_x - cur_x - 1, 2) + pow(goal_y - cur_y, 2));
